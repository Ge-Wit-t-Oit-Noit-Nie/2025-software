  ...
  for (;;)
    {
      ...
      switch (instruction[pcr.instruction_pointer].opcode)
        ...
        case OPCODE_PIN_TOGGLE:
          HAL_GPIO_TogglePin (
              (GPIO_TypeDef *)instruction[pcr.instruction_pointer].parameter1,
        ...
        case OPCODE_DELAY:
          ...
          else if ((state & EXTERN_INTERRUPT_EVENT_KILL)
          ...
          else if ((state & EXTERN_INTERRUPT_EVENT_PAUZE)
                   == EXTERN_INTERRUPT_EVENT_PAUZE)
            {
        ...
        case OPCODE_LOG_PROGRAM_STATE:
          {
            ...
            if (osOK
                != osMessageQueuePut (loggerQueueHandle, &telemetry, 0, 0U))
              {
                printf (
                    "Error: Could not send message to loggerQueueHandle\n\r");
              }
            program_controller_step (&pcr);
            break;
          }
...
HAL_GPIO_EXTI_Callback (uint16_t GPIO_Pin)
{
  ...
  if (PROGRAM_STATE_TRIGGER_Pin == GPIO_Pin)
    {
      osEventFlagsSet (ext_interrupt_eventHandle,
                       EXTERN_INTERRUPT_EVENT_PAUZE);
    }
}
 ...
 *
 * This function increments the program counter and wraps around if the end of
 * the program is reached.
 * @param  argument: Not used
 */
void
program_controller_step (
    program_controller_registers_t *program_controller_registers)
{
